#!/usr/bin/env python

import socket

TCP_IP = '192.168.0.115'
TCP_PORT = 3333
BUFFER_SIZE = 1024

crc_table = [ , ]

def calc_crc(data):
	crc = 0
	for ch in data:
		crc = crc_table[crc]^ord(ch);
	return crc

s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
s.connect((TCP_IP, TCP_PORT))

def msg_send(msg):
	msg = msg + chr(calc_crc(msg))
	print "sending", msg.encode("hex")
	s.send(msg)

#msg_send("\x31\x08\x07\x04\x00")
msg_send("\x31\x08\xb5\x09\x03\x0d\x05\x00")

chold = "a"
id = 0
doscan = False

while True:
  ch = s.recv(1)
  if ch != chr(0xaa):
    print ch.encode("hex"),
  else:
  	if chold != chr(0xaa):
  	  print "\n"
  	  if doscan:
  	  	msg_send("\x31"+chr(id)+"\x07\x04\x00")
  	  	id += 1
  chold = ch

s.close()

